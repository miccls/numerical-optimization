...
\begin{enumerate}[label = \roman*)]
    \item The gradient of a function is the vector of all its partial derivatives \be
    ...
    \end{pmatrix}
    \ee
    A symmetric matrix is positive definite if all it's leading principal minors are positive\footnote{See \href{https://en.wikipedia.org/wiki/Sylvester\%27s_criterion}{\textcolor{blue}{Sylvester's criterion}}}. The principal minors are the determinants of square sub-matrices starting from the upper left corner. 
...
&=2A_kx
\end{align*}
where $A_k$ is the $k^{\text{th}}$ column of $A$ and $\delta_{ij}$ is the Kronecker delta\footnote{See \href{https://en.wikipedia.org/wiki/Kronecker_delta}{\textcolor{blue}{Kronecker delta}}}. 
...
This means\footnote{If $S_i$ had been rows, we would have had $\nabla \tilde{f}(z) = S\nabla f(x)$}
\be
...
\ee
Given that the initial Hessians approximations are chosen s.t $\tilde{B}_0 = S^TB_0S$ we can see that
\be
...
\[
\kappa(A) = \frac{{\operatorname{max}}_i|\lambda_i|}{\operatorname{min}_i|\lambda_i|}
\]
